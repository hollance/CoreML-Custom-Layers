  ...
  func evaluate(inputs: [MLMultiArray], outputs: [MLMultiArray]) throws {
    ...
    for i in 0..<inputs.count {
      ...
      */
      ...
      // output = input * beta
      var beta = self.beta
      vDSP_vsmul(inputPointer, 1, &beta, outputPointer, 1, vDSP_Length(count))

      // output = -(input * beta)
      vDSP_vneg(outputPointer, 1, outputPointer, 1, vDSP_Length(count))

      // output = exp(-input * beta)
      var countAsInt32 = Int32(count)
      vvexpf(outputPointer, outputPointer, &countAsInt32)

      // output = 1 + exp(-input * beta)
      var one: Float = 1
      vDSP_vsadd(outputPointer, 1, &one, outputPointer, 1, vDSP_Length(count))

      // output = input / (1 + exp(-input * beta))
  ...
  func encode(commandBuffer: MTLCommandBuffer, inputs: [MTLTexture], outputs: [MTLTexture]) throws {
    ...
    if let encoder = commandBuffer.makeComputeCommandEncoder() {
      for i in 0..<inputs.count {
        ...
        // Pass the beta parameter to the compute kernel in an MTLBuffer.
        var beta = self.beta
